...
export default function FormAddProject({ open, onClose, id, isEdit, setIsEdit }){
 ...
 const handleSnackbarClose=()=>{
  ...
  async function saveProject(proyecto)  {

    return api.post('/proyectos', proyecto)
      .then(response => {
        const { idTransaccion: id } = response.data;  
        setIsEdit(true);
        setSnackbarMessage("Proyecto grabado exitosamente en la base de datos")
  ...
  const checkFormErrors=()=>{
    if (formErrors.idProyecto !=="" || formErrors.nombre !=="" || formErrors.descripcion !=="" ){
      return true
    }
    return false
  }
          ...
          >
            <DialogContent style={{color:'white'}}>
              <DialogContentText id="alert-dialog-description">
                <Typography align='center'>{`Incluir Nuevo Proyecto`}</Typography>
                ...
                <Grid item xs={12} >
                  <TextField variant ="outlined" margin="dense" size="small" fullWidth
                    ...
                    name="idProyecto"
                    value={idProyecto}
                    autoComplete="id-proyecto"
                    onChange={ (e) => {
                      handleChange (e,setProyecto,[noBlanks]);
                    }}
                    error={formErrors.idProyecto}
                  />
                  {formErrors.idProyecto ? <div className="error-helper-text">{formErrors.idProyecto}</div> : null}
                </Grid> 
                ...
                <Grid item xs={6} sm={6}>
                  <TextField variant ="outlined" margin="dense" size="small" fullWidth
                    ...
                    error={formErrors.metrosCuadrados}
                  />
                  {formErrors.metrosCuadrados ? <div className="error-helper-text">{formErrors.metrosCuadrados}</div> : null}
                </Grid>
                ...
                <Grid item xs={6} sm={6}>
                  <TextField variant ="outlined" margin="dense" size="small" fullWidth
                    ...
                    error={formErrors.margenEstimado}
                  />
                  {formErrors.margenEstimado ? <div className="error-helper-text">{formErrors.margenEstimado}</div> : null}
                </Grid>
                </Grid>
              </DialogContentText>
            </DialogContent>
