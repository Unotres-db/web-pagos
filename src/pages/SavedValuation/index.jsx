...
import TableHistoricalData from '../Valuation/TableHistoricalData'; // revisar
import FormOptions from '../Valuation/FormOptions'; 
import FormBusinessData from '../Valuation/FormBusinessData';
import FormCashFlowData from '../Valuation/FormCashFlowData';
...
import TableCashFlow from '../Valuation/TableCashFlow';
...
export default function SavedValuation () {
  ...
  const { axiosFetch: getValuation } = useAxios();
  const [ isEstimateFcffOnly, setIsEstimateFcffOnly ] = useState(assumptions.cashFlowGrowthRate !=="" && assumptions.revenueGrowth ===""? true: false );                  // Check if valuation assumptions will be complete or just estimate directly the cash flow growth
  ...
  const valuation = forecastedFinancialData ? calcValuation(companyData, historicalFinancialData, assumptions, calculatedCostOfCapital, forecastedFinancialData): {valuationId:"", cashFlowAvgGrowth:0, sumOfCashFlowPresentValue:0, perpetuityValue:0, perpetuityPresentValue:0, enterpriseValue:0, cash:0, debt:0, equityValue:0, sharesOutstanding:0, targetStockPrice:0, marketCap:0, updated_at:"",published:"", publishedDate:""} ;
  const [ Prompt, setIsDirty, setIsPristine ] = useUnsavedWarning();
  const [ savedValuationData, setSavedValuationData] = useState({id:"",updatedAt:"", published:"" })
  const [ editMode, setEditMode ] = useState(null);                                    // blank, edited, completed, saved, published
  ...
  function valuationSuccessCallback (apiData){
    setCompanyIdSearch(apiData.companyId)
    // console.log(apiData);
    if (apiData.cashFlowAvgGrowth > 0 && apiData.revenueGrowth ==="") {
    ...
    setSavedValuationData({id:apiData.valuationId,updatedAt:apiData.updated_at, published:apiData.published })
  }
  ...
  function errorCallback(errorMessage){
    setDialogOptions({severity:"error", title:"Hello Oops", message:errorMessage,buttons:{button1:"Ok"}})
  ...
  useEffect(() => {
    ...
    getValuation({ axiosInstance: valuationsWebApi, method: 'GET', url: `/valuations/${id}`, requestConfig: { headers: {'Authorization': userId,},}},valuationSuccessCallback, errorCallback);
  ...
  useEffect(() => {
    if (companyIdSearch){
      ...
      // setEditMode(checkValuationStatus(companyData, assumptions, isEstimateFcffOnly, savedValuationData));
    }
  }, [companyIdSearch]);
